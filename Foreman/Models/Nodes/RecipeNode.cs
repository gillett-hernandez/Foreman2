...
namespace Foreman
{
	public class RecipeNode : BaseNode
	{
		...
		public double GetSpeedMultiplier()
		{
			...
			{//this is a bit of a hack - by setting the speed multiplier here like so we get the # of buildings to be the # of rockets launched no matter the time scale.
	...
	public class RecipeNodeController : BaseNodeController
	{
		...
		{
			// if this recipe's fuel is not equal to the target fuel
			if (MyNode.Fuel != fuel
				// or it's null and the remains is not null
				|| (MyNode.Fuel == null && MyNode.FuelRemains != null)
				// or if it's not null but the burnresult doens't match the fuel remains
				|| (MyNode.Fuel != null && MyNode.Fuel.BurnResult != MyNode.FuelRemains))
			{
